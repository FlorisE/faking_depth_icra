...
sys.path.append('../')
from depth2depth_dataloader import get_train_dataset, get_valid_dataset


from dishwasher_depth2depth_shared import train_generator, \
                                         train_discriminator, \
                                         batch_it, \
                                         measure, \
                                         get_resnet_networks, \
...
MIN_CLIP=450
MAX_DEPTH=2000
STATS_PATH='dishwasher_depth2depth_resnet_wineglass.obj'
BEST_CHECKPOINT_PATH='../data/checkpoints/dishwasher-depth2depth-resnet-wineglass-best'
LATEST_CHECKPOINT_PATH='../data/checkpoints/dishwasher-depth2depth-resnet-wineglass-latest'
LOGS='./logs/dishwasher_depth2depth_resnet_wineglass/'
...
def calculate_validation_rmse(generator_t2o, generator_o2t, transparent_matching_list, opaque_matching_list):
    ...
    for i in range(len(transparent_matching_list)):
        ...
        t2o_d_rmses.append(np.sqrt(np.mean(np.square(opaque - generated_t2o))))
        o2t_d_rmses.append(np.sqrt(np.mean(np.square(transparent - generated_o2t))))
    return np.mean(t2o_d_rmses), np.mean(o2t_d_rmses)
...
def train(num_epochs, 
    ...
    for epoch in range(start_from, num_epochs):
        ...
        for transparent, opaque in dataset.shuffle(bs).prefetch(tf.data.AUTOTUNE).take(bs):
            ...
            gen_t2o_loss, gen_o2t_loss, cycle_loss_o, cycle_loss_t, total_cycle_loss, identity_loss_o2t, identity_loss_t2o, total_gen_t2o_loss, total_gen_o2t_loss, generated_t2o, generated_o2t = \
                train_generator(transparent, opaque, generator_t2o, generator_o2t, generator_t2o_optimizer, generator_o2t_optimizer, discriminator_t, discriminator_o, alpha_1)
            ...
            disc_t_loss, disc_o_loss = train_discriminator(transparent, generated_o2t, opaque, generated_t2o, discriminator_t, discriminator_o, discriminator_t_optimizer, discriminator_o_optimizer)
        ...
        t2o_d_rmse, o2t_d_rmse = calculate_validation_rmse(generator_t2o, generator_o2t, transparent_matching, opaque_matching)

        t2o_d_rmses.append(t2o_d_rmse)
        o2t_d_rmses.append(o2t_d_rmse)

        measure(transparent_matching, opaque_matching, masks_matching, generator_t2o, min_depth=min_depth, max_depth=max_depth, min_depth_valid=MIN_CLIP)
        
        if best_val_loss is None:
            print('Saving initial best validation loss')
            best_val_loss = t2o_d_rmse
            ckpt_manager_best.save()
        elif t2o_d_rmse < best_val_loss:
        ...
        #    tf.summary.scalar('mean_gen_t2o_losses_epoch', mean_gen_t2o_losses_epoch, step=epoch)
        #    tf.summary.scalar('mean_gen_o2t_losses_epoch', mean_gen_o2t_losses_epoch, step=epoch)
        #    tf.summary.scalar('mean_cycle_losses_o_epoch', mean_cycle_losses_o_epoch, step=epoch)
        #    tf.summary.scalar('mean_cycle_losses_t_epoch', mean_cycle_losses_t_epoch, step=epoch)
        #    tf.summary.scalar('mean_total_cycle_losses_epoch', mean_total_cycle_losses_epoch, step=epoch)
        #    tf.summary.scalar('mean_identity_losses_o2t_epoch', mean_identity_losses_o2t_epoch, step=epoch)
        #    tf.summary.scalar('mean_identity_losses_t2o_epoch', mean_identity_losses_t2o_epoch, step=epoch)
        #    tf.summary.scalar('mean_total_gen_o2t_losses_epoch', mean_total_gen_o2t_losses_epoch, step=epoch)
        #    tf.summary.scalar('mean_disc_t_losses_epoch', mean_disc_t_losses_epoch, step=epoch)
        #    tf.summary.scalar('mean_disc_o_losses_epoch', mean_disc_o_losses_epoch, step=epoch)
        #    tf.summary.scalar('t2o_d_rmse', t2o_d_rmse, step=epoch)
        #    tf.summary.scalar('o2t_d_rmse', o2t_d_rmse, step=epoch)
...
if __name__ == '__main__':
    generator_t2o, generator_o2t, discriminator_t, discriminator_o = get_resnet_networks(1, 1, 1, 1)
    
    generator_t2o_optimizer, generator_o2t_optimizer, discriminator_t_optimizer, discriminator_o_optimizer = get_optimizers()
    ...
    transparent_matching, opaque_matching, masks_matching = get_valid_dataset(min_depth=MIN_DEPTH, min_clip=MIN_CLIP)
